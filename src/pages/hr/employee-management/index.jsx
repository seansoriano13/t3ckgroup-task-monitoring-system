...
import { Search, UserPlus, Edit, Trash2, Shield, Loader2, XSquare } from "lucide-react";
    ...
    <ProtectedRoute requireHr={true}>
      <div className="max-w-7xl mx-auto space-y-6 pb-10 px-2 sm:px-0">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-gray-4 pb-4">
          <div>
            <h1 className="text-3xl font-black text-gray-12">Employee Management</h1>
        ...
        <div className="bg-gray-2 border border-gray-4 p-4 rounded-xl flex gap-4 shadow-sm relative z-20">
          <div className="relative flex-1">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-8" size={18} />
            <input
              type="text"
            ...
            <tbody className="divide-y divide-gray-4">
              {isLoading ? (
                ...
                filteredEmployees.map((emp) => (
                  <tr key={emp.id} className="hover:bg-gray-3/30 transition-colors">
                    <td className="p-4 text-sm font-bold text-gray-12">{emp.name}</td>
                    <td className="p-4 text-sm text-gray-10">{emp.email}</td>
                    <td className="p-4 text-sm text-gray-11">{emp.department || "-"}</td>
                    <td className="p-4 text-sm text-gray-11">{emp.subDepartment || "-"}</td>
                    <td className="p-4 text-xs flex gap-1 items-center">
                      {emp.isSuperAdmin && (
                        <span className="bg-purple-900/30 text-purple-400 border border-purple-900/50 px-2 py-1 rounded font-bold flex items-center gap-1">
                          <Shield size={12} /> Super Admin
                        </span>
                      )}
                      {emp.isHr && (
                        <span className="bg-blue-900/30 text-blue-400 border border-blue-900/50 px-2 py-1 rounded font-bold">
                          HR
                        </span>
                      )}
                      {emp.isHead && (
                        <span className="bg-amber-900/30 text-amber-400 border border-amber-900/50 px-2 py-1 rounded font-bold">
                          Head
                        </span>
                      )}
                ...
                <tr>
                  <td colSpan="6" className="p-8 text-center text-gray-9 font-bold">
                    No employees found.
                  </td>
                </tr>
              )}
...
function EmployeeFormModal({ employee, onClose }) {
  ...
  const uniqueDepts = useMemo(() => {
    return [...new Set(rawCategories.map((c) => c.department).filter(Boolean))].sort();
  }, [rawCategories]);
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-gray-1 border border-gray-4 rounded-xl w-full max-w-md shadow-2xl p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="text-xs font-bold text-gray-9 uppercase">Full Name</label>
            <input
              required
              type="text"
              value={formData.name}
          ...
          <div>
            <label className="text-xs font-bold text-gray-9 uppercase">Google Auth Email</label>
            <input
              required
              type="email"
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-bold text-gray-9 uppercase">Department</label>
              <select
                required
            ...
            <div>
              <label className="text-xs font-bold text-gray-9 uppercase">Sub-Dept</label>
              <select
                required
          ...
          <div className="pt-2 border-t border-gray-4 space-y-2">
             <label className="text-xs font-bold text-gray-9 uppercase block mb-2">Role Assignments</label>
             <label className="flex items-center gap-2 text-sm text-gray-12 font-medium cursor-pointer">
               <input type="checkbox" checked={formData.isHead} onChange={(e) => setFormData({...formData, isHead: e.target.checked})} className="rounded text-primary focus:ring-primary h-4 w-4 bg-gray-2 border-gray-4" />
               Department Head
             </label>
             <label className="flex items-center gap-2 text-sm text-gray-12 font-medium cursor-pointer">
               <input type="checkbox" checked={formData.isHr} onChange={(e) => setFormData({...formData, isHr: e.target.checked})} className="rounded text-primary focus:ring-primary h-4 w-4 bg-gray-2 border-gray-4" />
               HR Administrator
             </label>
             <label className="flex items-center gap-2 text-sm text-gray-12 font-medium cursor-pointer">
               <input type="checkbox" checked={formData.isSuperAdmin} onChange={(e) => setFormData({...formData, isSuperAdmin: e.target.checked})} className="rounded text-primary focus:ring-primary h-4 w-4 bg-gray-2 border-gray-4" />
               Super Admin
             </label>
          </div>
