...
export default function LogRevenuePage() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setFormData(f => ({ ...f, account: '', product_item_sold: '', revenue_amount: '', so_number: '', reference_number: '', remarks: '' }));
      ...
      <div className="max-w-3xl mx-auto space-y-6 pb-10 px-2 sm:px-4">
        ...
        <form onSubmit={handleSubmit} className="bg-gray-1 border border-gray-4 p-6 sm:p-8 rounded-2xl shadow-xl space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div>
              <label className="text-xs font-bold text-gray-9 uppercase block mb-1">Payment Date</label>
            ...
            <div>
              ...
              <div className="flex bg-gray-2 border border-gray-4 rounded-lg p-1">
                <button
                  ...
                  className={`flex-1 py-2 px-3 text-xs font-black uppercase tracking-widest rounded-md transition-all ${formData.status === 'COMPLETED' ? 'bg-green-600 text-white shadow-md' : 'text-gray-9 hover:text-gray-12'}`}
                >
                  COMPLETED
                </button>
                <button
            ...
            <div className="sm:col-span-1">
              <label className="text-xs font-bold text-gray-9 uppercase block mb-1">SO Number <span className="normal-case font-medium text-gray-8">(Optional)</span></label>
              <input type="text" placeholder="e.g. SO-2026-0042" value={formData.so_number} onChange={e => setFormData({ ...formData, so_number: e.target.value })} className="w-full bg-gray-2 border border-gray-4 text-gray-12 rounded-lg p-3 outline-none focus:border-green-500" />
            </div>
