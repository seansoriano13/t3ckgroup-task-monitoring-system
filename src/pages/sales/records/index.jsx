...
import { Search, Briefcase, FileText, CheckCircle2, Circle, AlertCircle, DollarSign, LayoutList, Table2, Columns, Users, Edit, X, Save, Lock, Unlock, MessageSquare } from "lucide-react";
...
export default function SalesRecordsPage() {
  ...
  useEffect(() => {
     if (location.state?.openRevenueId && rawRevenue.length > 0) {
        const targetRev = rawRevenue.find(r => r.id === location.state.openRevenueId);
        if (targetRev) {
        ...
        if (eventType === 'SALES_PLAN_SUBMITTED') {
           targetAct = rawActivities.find(a => String(a.plan_id) === String(location.state.openEventId));
        } else {
           targetAct = location.state.openEventId ? rawActivities.find(a => String(a.id) === String(location.state.openEventId)) : null;
        }
        
        const targetEmp = location.state.fallbackEmpId || targetAct?.employee_id;
        const targetDate = targetAct ? targetAct.scheduled_date : location.state.fallbackDate;
  ...
  const filteredActivities = useMemo(() => {
    ...
    if (filterType !== "ALL") filtered = filtered.filter(a => {
    ...
    return filtered;
  }, [allowedActivities, filterEmp, filterStatus, filterType, searchTerm, selectedDateFilter, timeframe]);
  ...
  const filteredRevenue = useMemo(() => {
    ...
    if (filterStatus !== "ALL") {
       ...
       if (filterStatus === 'DONE') filtered = filtered.filter(a => (a.status.includes('COMPLETED') || a.status === 'Won') && (!isVerificationEnforced || a.is_verified !== false));
       if (filterStatus === 'INCOMPLETE') filtered = filtered.filter(a => a.status.includes('LOST') || a.status === 'Lost');
       if (filterStatus === 'UNVERIFIED') filtered = filtered.filter(a => a.is_verified === false);
    }
    if (searchTerm) {
      const lower = searchTerm.toLowerCase();
      filtered = filtered.filter(a => 
  ...
  const boardData = useMemo(() => {
    ...
    filteredActivities.forEach(act => {
       ...
       if (timeframe === "MONTHLY") dateStr = act.scheduled_date.substring(0, 7); // YYYY-MM
       else if (timeframe === "YEARLY") dateStr = act.scheduled_date.substring(0, 4); // YYYY
       ...
       if (!byEmp[empName][dateStr]) byEmp[empName][dateStr] = { AM: [], PM: [], all: [] };
    ...
    // 2. Convert to array
    return Object.entries(byEmp).map(([empName, datesMap]) => {
       const sortedDates = Object.entries(datesMap).sort(([d1], [d2]) => d1.localeCompare(d2)).map(([dateStr, blocks]) => ({
          dateStr,
          AM: blocks.AM,
          PM: blocks.PM,
          all: blocks.all
      ...
      <div className="max-w-[1600px] mx-auto space-y-6 pb-10 px-2 sm:px-4">
        ...
        <div className="bg-gray-1 border border-gray-4 rounded-xl p-4 shadow-sm flex flex-col md:flex-row gap-4">
          ...
          <div className="flex gap-2 flex-wrap sm:flex-nowrap">
            {viewMode === "BOARD" && activeTab === "ACTIVITIES" && (
                <div className="flex bg-gray-2 border border-gray-4 rounded-lg overflow-hidden flex-1 sm:flex-none shadow-sm focus-within:border-primary transition-colors">
                  ...
                  <DatePicker 
                    ...
                    dateFormat={timeframe === "YEARLY" ? "yyyy" : timeframe === "MONTHLY" ? "MMMM yyyy" : "MMM d, yyyy"}
                    isClearable={true}
                    placeholderText={timeframe === "YEARLY" ? "Select Year..." : timeframe === "MONTHLY" ? "Select Month..." : "Select Date..."}
            ...
            >
              ...
              {user?.isSuperAdmin && uniqueEmployees.map(emp => (
            ...
            >
              ...
              {activeTab === 'ACTIVITIES' ? (
                 ...
                 <>
                  ...
                  {isVerificationEnforced && <option value="UNVERIFIED">Pending Verification</option>}
                 </>
              )}
            </select>
        ...
        {activeTab === "ACTIVITIES" && viewMode === "BOARD" && (
           <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
              {boardData.length === 0 ? (
                 <div className="p-10 text-center text-gray-9 font-bold bg-gray-1 rounded-2xl border border-gray-4"><AlertCircle className="mx-auto mb-2 text-gray-8"/>No activities match the filters.</div>
              ) : (
                 boardData.map(empGroup => (
                    <div key={empGroup.employeeName} className="bg-gray-1 border border-gray-4 rounded-2xl p-4 sm:p-6 shadow-sm">
                       <h2 className="text-xl font-black text-gray-12 mb-4 border-b border-gray-4 pb-2 flex items-center gap-2">
                           <Users size={20} className="text-primary" /> {empGroup.employeeName}
                       </h2>
                        <div className="flex overflow-x-auto gap-4 sm:gap-6 pb-4 custom-scrollbar snap-x">
                           {empGroup.dates.map(dateBlock => {
                              ...
                              if (timeframe === 'MONTHLY' || timeframe === 'YEARLY') {
                                 ...
                                 const label = timeframe === 'MONTHLY' ? new Date(dateBlock.dateStr + '-01').toLocaleDateString('en-US', { month: 'long', year: 'numeric' }) : dateBlock.dateStr;

                                 return (
                                     <div key={dateBlock.dateStr} className="min-w-[200px] shrink-0 bg-gray-2 rounded-xl border border-gray-4 p-5 snap-start shadow-sm flex flex-col justify-center text-center transition-transform hover:-translate-y-1">
                                         <h3 className="font-black text-gray-12 mb-2 bg-gray-3 px-3 py-1.5 rounded-lg text-sm uppercase tracking-widest truncate" title={label}>{label}</h3>
                                         <p className="text-4xl font-black text-primary my-3">{total} <span className="text-[10px] text-gray-9 uppercase tracking-widest block font-bold mt-1">Total Operations</span></p>
                                         <div className="mt-auto pt-4 border-t border-gray-4 flex justify-between items-center w-full px-2">
                                            <span className="text-xs font-bold text-gray-10 uppercase">Completed</span>
                                            <span className={`text-sm font-black text-white px-2 py-0.5 rounded-full ${pct >= 80 ? 'bg-green-500' : pct >= 50 ? 'bg-yellow-500' : 'bg-red-500'}`}>{pct}%</span>
                                         </div>
                                     </div>
                                 )
                              }
                              ...
                              return (
                                 <div key={dateBlock.dateStr} className="min-w-[280px] sm:min-w-[320px] w-[280px] sm:w-[320px] shrink-0 bg-gray-2 rounded-xl border border-gray-4 p-4 flex flex-col snap-start">
                                    <h3 className="font-bold text-gray-12 mb-3 bg-gray-3 px-3 py-1.5 rounded-lg text-center font-mono text-sm tracking-wide truncate" title={dateBlock.dateStr}>
                                       {timeframe === 'WEEKLY' ? dateBlock.dateStr : `${new Date(dateBlock.dateStr).toLocaleDateString('en-US', { weekday: 'long' })} - ${dateBlock.dateStr}`}
                                    </h3>
                                    
                                    <div className="space-y-4 flex-1 flex flex-col">
                                       <div className="flex-1 bg-white dark:bg-gray-3 rounded-lg border border-gray-4 p-3 h-full">
                                          <h4 className="flex items-center gap-2 text-[10px] font-black text-gray-10 uppercase tracking-widest mb-2 border-b border-gray-4 pb-1">AM Block</h4>
                                          <div className="space-y-2">
                                             {dateBlock.AM.length === 0 && <p className="text-xs text-gray-8 italic text-center py-2">No AM Tasks</p>}
                                             {dateBlock.AM.map(act => (
                                                <BoardActivityCard key={act.id} act={act} onClick={() => setSelectedActivity(act)} />
                                             ))}
                                          </div>
                                       </div>
                                       
                                       <div className="flex-1 bg-white dark:bg-gray-3 rounded-lg border border-gray-4 p-3 h-full">
                                          <h4 className="flex items-center gap-2 text-[10px] font-black text-gray-10 uppercase tracking-widest mb-2 border-b border-gray-4 pb-1">PM Block</h4>
                                          <div className="space-y-2">
                                             {dateBlock.PM.length === 0 && <p className="text-xs text-gray-8 italic text-center py-2">No PM Tasks</p>}
                   ...
                   <tbody className="divide-y divide-gray-4">
                      {isRevLoading ? (
                         ...
                         filteredRevenue.map((log) => (
                            <tr key={log.id} onClick={() => setEditingRevenue(log)} className="hover:bg-gray-3/50 transition-colors cursor-pointer" title="Click to Edit Revenue Log">
                               ...
                               <td className="p-4 text-center">
                                 {isVerificationEnforced && log.is_verified === false ? (
                                   <span className="bg-orange-500/10 text-orange-500 border border-orange-500/20 px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-widest">PENDING</span>
                                 ) : log.status === 'COMPLETED SALES' || log.status === 'Won' ? (
                                   <span className="bg-green-500/10 text-green-500 border border-green-500/20 px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-widest">COMPLETED</span>
                                 ) : (
                                   <span className="bg-red-500/10 text-red-500 border border-red-500/20 px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-widest">LOST</span>
                                 )}
                               </td>
                            </tr>
                         ))
  ...
  return (
    <div 
      ...
      className={`p-2.5 rounded-lg border text-left cursor-pointer transition-all hover:-translate-y-0.5 shadow-sm
        ${isDone ? 'bg-green-500/10 border-green-500/20' : 'bg-gray-2 border-gray-4 hover:border-primary/50'}
        ${act.is_unplanned ? 'border-l-4 border-l-blue-500' : ''}
      `}
    >
      <p className={`text-[13px] font-bold truncate ${isDone ? 'text-gray-9 line-through' : 'text-gray-12'}`} title={act.account_name}>{act.account_name || <span className="text-gray-8 italic">No Account</span>}</p>
      <div className="flex items-center justify-between mt-1">
         <span className="text-[9px] uppercase font-bold text-gray-10 truncate max-w-[120px]">{act.activity_type}</span>
         {isDone ? (
...
const EditRevenueModal = ({ isOpen, onClose, log, onSave, isSaving, currentUser, isVerificationEnforced }) => {
  if (!isOpen || !log) return null;

  const isVerifiedAndLocked = isVerificationEnforced && log.is_verified === true && !currentUser?.isSuperAdmin;
  const hasPendingRequest = isVerificationEnforced && log.edit_request_status === 'PENDING';
  ...
  const requestMutation = useMutation({
    mutationFn: (payload) => salesService.requestRevenueEdit(log.id, payload.amount, payload.reason, currentUser.id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["allRevenueLogs"] });
  ...
  const resolveMutation = useMutation({
    mutationFn: ({ isApproved }) => salesService.resolveEditRequest(log.id, isApproved, log.edit_request_amount, currentUser.id),
    onSuccess: (data, variables) => {
      queryClient.invalidateQueries({ queryKey: ["allRevenueLogs"] });
      toast.success(variables.isApproved ? "Change Approved & Applied!" : "Request Rejected");
      onClose();
    },
    onError: (err) => toast.error(err.message)
  });
  ...
  return (
     <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
       <div className="bg-gray-1 border border-gray-4 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
          ...
          <div className="p-4 border-b border-gray-4 flex justify-between items-center bg-gray-2 shrink-0">
             <h2 className="text-xl font-black text-gray-12 flex items-center gap-2">
               {isVerifiedAndLocked ? <Lock size={20} className="text-primary"/> : <Edit size={20} className="text-green-500"/>} 
               {isVerifiedAndLocked ? 'Locked Log' : 'Edit Revenue'}
             </h2>
             <button onClick={onClose} className="p-2 bg-gray-3 hover:bg-gray-4 rounded-full text-gray-10 transition-colors"><X size={16} /></button>
          </div>
          ...
          <div className="overflow-y-auto p-6 space-y-4 custom-scrollbar">
             ...
             {currentUser?.isSuperAdmin && hasPendingRequest && (
                <div className="bg-blue-500/10 border border-blue-500/30 rounded-xl p-4 shadow-sm mb-2">
                   <div className="flex items-start gap-3">
                      <div className="bg-blue-500/20 p-2 rounded-lg text-blue-500 mt-0.5"><MessageSquare size={16}/></div>
                      <div className="flex-1">
                         <h4 className="text-xs font-black text-blue-600 uppercase tracking-widest mb-1">Edit Request Pending</h4>
                         <p className="text-sm text-gray-12 bg-white dark:bg-gray-3 p-2 rounded-lg border border-gray-4 font-medium italic my-2">"{log.edit_request_reason}"</p>
                         <div className="flex items-center gap-2 mt-2">
                            <span className="text-[10px] uppercase font-bold text-gray-9">Proposed Override:</span>
                            <span className="text-sm font-black text-blue-600 line-through opacity-60">₱{Number(log.revenue_amount).toLocaleString()}</span>
                            <span className="text-xs font-black text-gray-9">→</span>
                            <span className="text-sm font-black text-green-600 bg-green-500/10 px-2 py-0.5 rounded border border-green-500/20">₱{Number(log.edit_request_amount).toLocaleString()}</span>
                         </div>
                      </div>
                   </div>
                   <div className="flex gap-2 mt-4 pt-3 border-t border-blue-500/20">
                      <button onClick={() => resolveMutation.mutate({ isApproved: false })} disabled={resolveMutation.isPending} className="flex-1 bg-gray-3 hover:bg-red-500/10 hover:text-red-500 text-gray-11 font-bold text-xs py-2 rounded-lg transition-colors">Reject Request</button>
                      <button onClick={() => resolveMutation.mutate({ isApproved: true })} disabled={resolveMutation.isPending} className="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-black text-xs py-2 rounded-lg shadow-md transition-colors">Approve & Apply Overlay</button>
                   </div>
                </div>
             )}
             ...
             {isVerifiedAndLocked && !requestMode && !hasPendingRequest && (
                <div className="bg-primary/5 border border-primary/20 rounded-xl p-4 text-center mb-6 shadow-sm">
                   ...
                   <p className="text-sm font-bold text-gray-12">This log has been audited and locked by a Super Admin.</p>
                   <p className="text-xs text-gray-9 mt-1 mb-4">Direct edits are disabled to protect dashboard integrity. If you made an error, you must submit an Edit Request.</p>
                   <button onClick={() => setRequestMode(true)} className="bg-primary hover:bg-primary/90 text-white font-black text-xs px-6 py-2.5 rounded-lg uppercase tracking-widest shadow-md transition-transform active:scale-95">Request Changes</button>
                </div>
             )}
             ...
             {isVerifiedAndLocked && hasPendingRequest && (
                <div className="bg-orange-500/5 border border-orange-500/20 rounded-xl p-4 text-center mb-6">
                   ...
                   <p className="text-sm font-bold text-orange-600">Edit Request Pending Review...</p>
                   <p className="text-xs text-gray-9 mt-1">A Super Admin has been notified of your proposed change to <strong className="text-gray-12">₱{Number(log.edit_request_amount).toLocaleString()}</strong>.</p>
                </div>
             )}

             {isVerifiedAndLocked && requestMode && (
                <form onSubmit={handleRequestSubmit} className="bg-gray-2 border border-blue-500/30 rounded-xl p-5 mb-6 animate-in fade-in zoom-in-95 duration-200">
                   <div className="flex items-center justify-between mb-4 border-b border-gray-4 pb-2">
                      <h4 className="text-xs font-black text-blue-600 uppercase tracking-widest flex items-center gap-1.5"><MessageSquare size={14}/> Submit Request</h4>
                      <button type="button" onClick={() => setRequestMode(false)} className="text-[10px] font-bold text-gray-9 hover:text-gray-12 uppercase">Cancel</button>
                   </div>
                   
                   <div className="space-y-4">
                      <div>
                         <label className="text-[10px] font-bold text-gray-9 uppercase mb-1 block">Proposed New Amount (PHP)</label>
                         <div className="relative">
                           <DollarSign size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-9" />
                           <input type="number" step="0.01" required value={requestData.amount} onChange={e => setRequestData({...requestData, amount: e.target.value})} className="w-full bg-white dark:bg-gray-3 border border-blue-500/40 rounded-lg pl-8 pr-3 py-2 text-sm font-black text-blue-600 outline-none focus:border-blue-500 shadow-inner" />
                         </div>
                      </div>
                      <div>
                         <label className="text-[10px] font-bold text-gray-9 uppercase mb-1 block">Reason for Edit <span className="text-red-500">*</span></label>
                         <textarea required placeholder="e.g. Typographical error during data entry, actual amount should be..." value={requestData.reason} onChange={e => setRequestData({...requestData, reason: e.target.value})} className="w-full bg-white dark:bg-gray-3 border border-gray-4 rounded-lg px-3 py-2 text-sm text-gray-12 outline-none focus:border-blue-500 resize-none h-20" />
                      </div>
                      <button disabled={requestMutation.isPending} type="submit" className="w-full bg-blue-600 hover:bg-blue-700 text-white font-black py-2.5 text-sm rounded-lg shadow-md transition-colors disabled:opacity-50">
                         {requestMutation.isPending ? 'Submitting...' : 'Send Request to Admin'}
                      </button>
                   </div>
                </form>
             )}
             ...
             <form id="standard-edit-form" onSubmit={handleSubmit} className={`space-y-4 ${isVerifiedAndLocked ? 'opacity-50 pointer-events-none grayscale-[50%]' : ''}`}>
                <div className="grid grid-cols-2 gap-4">
                   <div>
                     <label className="text-[10px] font-bold text-gray-9 uppercase mb-1 block">Date</label>
                     <input type="date" required value={formData.date} onChange={e => setFormData({...formData, date: e.target.value})} className="w-full bg-gray-2 border border-gray-4 rounded-lg px-3 py-2 text-sm text-gray-12 outline-none focus:border-primary font-bold" />
                   </div>
                   <div>
                     <label className="text-[10px] font-bold text-gray-9 uppercase mb-1 block">Status</label>
                     <select required value={formData.status} onChange={e => setFormData({...formData, status: e.target.value})} className="w-full bg-gray-2 border border-gray-4 rounded-lg px-3 py-2 text-sm text-gray-12 outline-none focus:border-primary font-bold">
                ...
                <div>
                   <label className="text-[10px] font-bold text-gray-9 uppercase mb-1 block">Account</label>
                   <input type="text" required value={formData.account} onChange={e => setFormData({...formData, account: e.target.value})} className="w-full bg-gray-2 border border-gray-4 rounded-lg px-3 py-2 text-sm text-gray-12 outline-none focus:border-primary" />
                </div>
                
                <div>
                   <label className="text-[10px] font-bold text-gray-9 uppercase mb-1 block">Product Sold</label>
                   <input type="text" required value={formData.product_item_sold} onChange={e => setFormData({...formData, product_item_sold: e.target.value})} className="w-full bg-gray-2 border border-gray-4 rounded-lg px-3 py-2 text-sm text-gray-12 outline-none focus:border-primary" />
                </div>

                <div>
                   <label className="text-[10px] font-bold text-gray-9 uppercase mb-1 block">Revenue Amount</label>
                   <div className="relative">
                      <DollarSign size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-9" />
                      <input type="number" step="0.01" required value={formData.revenue_amount} onChange={e => setFormData({...formData, revenue_amount: e.target.value})} className={`w-full bg-gray-2 border border-gray-4 rounded-lg pl-8 pr-3 py-2 text-sm font-black outline-none focus:border-green-500 shadow-inner ${isVerifiedAndLocked ? 'text-gray-12' : 'text-green-600'}`} />
                   </div>
                </div>

                 {currentUser?.isSuperAdmin && isVerificationEnforced && log.is_verified === false && !hasPendingRequest && (
                    <div className={`border rounded-xl p-4 flex flex-col sm:flex-row gap-3 sm:items-center justify-between transition-colors ${formData.is_verified ? 'bg-green-500/10 border-green-500/30' : 'bg-orange-500/5 border-orange-500/20'}`}>
                       <div>
                         <p className={`text-[10px] font-black uppercase tracking-widest flex items-center gap-1.5 ${formData.is_verified ? 'text-green-600' : 'text-orange-500'}`}>{formData.is_verified ? 'Audit Complete' : 'Pending Verification'}</p>
                         <p className="text-xs text-gray-9 mt-0.5 font-bold max-w-sm leading-tight">This log is trapped and requires a Super Admin stamp to be globally applied to Dashboard metrics.</p>
                       </div>
                       <button 
                          type="button" 
                 ...
                 {currentUser?.isSuperAdmin && isVerificationEnforced && log.is_verified === true && !hasPendingRequest && (
                    <div className="bg-green-500/5 border border-green-500/20 rounded-xl p-3 flex sm:items-center justify-between gap-3">
                       <span className="text-[10px] font-black text-green-600 uppercase tracking-widest flex items-center gap-1.5"><CheckCircle2 size={14}/> Verified & Locked</span>
                       <button type="button" onClick={() => setFormData({...formData, is_verified: false})} className="px-3 py-1.5 font-bold text-[10px] uppercase tracking-widest rounded-md bg-gray-3 hover:bg-orange-500/20 hover:text-orange-500 text-gray-11 transition-colors flex items-center gap-1"><Unlock size={12}/> Unverify</button>
                    </div>
                 )}

                <div>
                   <label className="text-[10px] font-bold text-gray-9 uppercase mb-1 block">Remarks</label>
                   <textarea disabled={isSaving} value={formData.remarks} onChange={e => setFormData({...formData, remarks: e.target.value})} className="w-full bg-gray-2 border border-gray-4 rounded-lg px-3 py-2 text-sm text-gray-12 outline-none focus:border-primary resize-none h-20 placeholder:text-gray-7" placeholder="Add optional details..." />
                </div>
                ...
                {log.editor?.name && (
                   <div className="bg-gray-3/50 px-3 py-2 rounded-lg border border-gray-4 mt-2 flex flex-col sm:flex-row sm:items-center justify-between gap-1">
                      <span className="text-[10px] font-black uppercase text-gray-10 tracking-widest inline-flex items-center gap-1.5"><Edit size={12}/> AUDIT LOG</span>
                      <span className="text-[11px] font-mono text-gray-11"><span className="font-bold text-gray-12">{log.editor.name}</span> @ {new Date(log.last_edited_at).toLocaleString('en-US', { month: 'short', day: 'numeric', year: 'numeric', hour: 'numeric', minute: '2-digit'})}</span>
                   </div>
                )}
             </form>
          ...
          <div className="p-4 border-t border-gray-4 bg-gray-2 shrink-0">
             {!isVerifiedAndLocked ? (
                <button type="submit" form="standard-edit-form" disabled={isSaving} className="w-full bg-green-600 hover:bg-green-700 text-white font-black py-3 rounded-xl flex items-center justify-center gap-2 transform active:scale-[0.98] transition-all disabled:opacity-50">
                    {isSaving ? "Saving..." : "Update Details"}
                </button>
             ) : (
                <button type="button" onClick={onClose} className="w-full bg-gray-4 hover:bg-gray-5 text-gray-12 font-black py-3 rounded-xl flex items-center justify-center gap-2 transition-all">
                    Close View
                </button>
             )}
          </div>
