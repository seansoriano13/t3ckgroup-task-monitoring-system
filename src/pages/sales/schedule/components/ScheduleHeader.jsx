...
export function ScheduleHeader({
  ...
  const totalDaysRequired =
    5 + (includeSaturday ? 1 : 0) + (includeSunday ? 1 : 0);
  const readinessPercent = Math.min(
    100,
    Math.round((weekSummary.daysReady / totalDaysRequired) * 100),
  );
  ...
  return (
    <div className="space-y-5 border-b border-border pb-6 mt-2">
      ...
      >
        <span
          className={`text-[10px] font-black px-3 py-1.5 rounded-xl border uppercase tracking-[0.2em] flex items-center shadow-sm ${
            plan.status === "APPROVED" || plan.status === "SUBMITTED"
              ? "bg-green-2 text-green-11 border-green-6"
              : plan.status === "REVISION"
                ? "bg-violet-2 text-violet-11 border-mauve-5"
                : plan.status === "LOCKED"
                  ? "bg-amber-2 text-amber-11 border-amber-6"
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pt-1">
        <div className="flex items-center gap-4 flex-wrap">
          {/* Date Picker */}
          <div className="bg-card border border-border rounded-xl px-4 py-2.5 flex items-center gap-2 shadow-sm transition-colors">
          ...
          {!isLocked && (
            <div className="flex items-center gap-4 bg-card border border-border rounded-xl px-4 py-2.5 shadow-sm">
              <label className="flex items-center gap-2 cursor-pointer">
                <div
                  className={`relative inline-flex h-4 w-7 items-center rounded-full transition-colors ${compactMode ? "bg-primary" : "bg-mauve-5"}`}
                >
                  <span
                    className={`inline-block h-3 w-3 transform rounded-full bg-card shadow transition-transform ${compactMode ? "translate-x-[14px]" : "translate-x-[2px]"}`}
                  />
                </div>
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                <div
                  className={`relative inline-flex h-4 w-7 items-center rounded-full transition-colors ${includeSaturday ? "bg-primary" : "bg-mauve-5"}`}
                >
                  <span
                    className={`inline-block h-3 w-3 transform rounded-full bg-card shadow transition-transform ${includeSaturday ? "translate-x-[14px]" : "translate-x-[2px]"}`}
                  />
                </div>
                <span className="text-xs font-bold text-muted-foreground select-none">
                  + Weekends
