    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-card rounded-2xl w-full max-w-sm overflow-hidden shadow-xl animate-in fade-in zoom-in duration-200">
        <div className="p-4 border-b border-mauve-3 flex justify-between items-center bg-mauve-1">
          ...
          <button onClick={onClose} className="text-mauve-8 hover:text-mauve-11 transition-colors">
            <X size={20} />
          </button>
        </div>
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          <div>
            <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider block mb-1">Template Name</label>
            <input
              autoFocus
              type="text"
              value={templateName}
              ...
              className="w-full bg-mauve-2 border border-mauve-4 rounded-xl px-3 py-2 text-sm text-foreground outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all"
            />
          </div>
          <div className="pt-2 flex justify-end gap-2">
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2 text-xs font-bold text-muted-foreground hover:bg-mauve-2 rounded-xl transition-colors"
            >
              CANCEL
            </button>
            <button
      ...
      <div className="bg-card rounded-2xl w-full max-w-lg overflow-hidden shadow-xl animate-in fade-in zoom-in duration-200">
        ...
        <div className="p-5">
          {customTemplates.length === 0 ? (
            <div className="text-center text-sm text-mauve-8 py-8">
              No custom templates found. <br/> Use the 3-dot menu on any activity to save one.
            </div>
          ) : (
            <div className="space-y-3 max-h-[60vh] overflow-y-auto pr-2">
              {customTemplates.map((template) => (
                <div key={template.id} className="flex justify-between items-center p-3 border border-mauve-3 rounded-xl hover:border-mauve-5 transition-colors bg-mauve-1/50">
                  <div className="min-w-0 pr-4">
                    <p className="font-bold text-sm text-foreground truncate">{template.template_name}</p>
                    <p className="text-xs text-mauve-8 truncate">
