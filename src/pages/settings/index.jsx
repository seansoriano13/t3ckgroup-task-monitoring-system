...
export default function SettingsPage() {
  ...
  const mutation = useMutation({
     mutationFn: (val) => salesService.updateAppSettings({ require_revenue_verification: val }),
     onSuccess: () => {
           ...
           <div className="bg-gray-2 border border-primary/30 rounded-2xl shadow-lg overflow-hidden relative">
              ...
              <div className="p-5 border-b border-gray-3 bg-gray-1 flex items-center gap-2 relative z-10">
                ...
                <h2 className="text-sm font-bold text-gray-12 uppercase tracking-wider">
                  Magician Security Core
                </h2>
              </div>
              <div className="p-5 flex items-center justify-between hover:bg-gray-1/50 transition-colors relative z-10">
                <div className="flex items-center gap-4">
                  <div className="text-gray-9">
                    <Shield size={20} className={appSettings?.require_revenue_verification ? 'text-primary' : ''} />
                  </div>
                  <div>
                    <p className="font-bold text-gray-12">Enforce Revenue Verification</p>
                    <p className="text-sm text-gray-9 max-w-sm">
                      Toggle whether sales personnel require an active Super Admin verification stamp upon logging revenue before it affects metrics.
                    </p>
                  </div>
                </div>
                {/* Custom Magician Toggle Switch */}
                {loadingSettings ? <Loader2 size={16} className="animate-spin text-gray-9" /> : (
                    <div
                      onClick={() => !mutation.isPending && mutation.mutate(!appSettings?.require_revenue_verification)}
                      className={`w-11 h-6 rounded-full relative cursor-pointer transition-colors ${
                        appSettings?.require_revenue_verification ? "bg-primary" : "bg-gray-4"
                      } ${mutation.isPending && 'opacity-50 cursor-not-allowed'}`}
                    >
                      <div className={`absolute top-1 bg-white w-4 h-4 rounded-full transition-all ${
                          appSettings?.require_revenue_verification ? "right-1" : "left-1"
                        }`}></div>
                    </div>
                )}
              </div>
