...
export default function SalesPerformanceMetrics({ selectedMonth }) {
  ...
  const expenseSummary = useMemo(() => {
    ...
    const avgWithRef =
      withRefCount > 0 ? Math.round(withRefExpense / withRefCount) : 0;
    const avgBizDev =
      bizDevCount > 0 ? Math.round(bizDevExpense / bizDevCount) : 0;
      ...
      {isAdminView && expenseSummary && (
        <div className="bg-gray-1 border border-gray-4 p-6 sm:p-8 rounded-3xl shadow-xl space-y-8">
            ...
            <div className="flex flex-wrap gap-3">
              <div className="bg-gray-2 px-4 py-2 border border-gray-4 rounded-xl">
                ...
                <p className="text-lg font-black text-gray-12">
                  ...
                  <span className="text-[10px] text-gray-9 font-medium">
                    Items
                  </span>
                </p>
              </div>
              <div className="px-4 py-2 border border-gray-4 rounded-xl">
                ...
                <p className="text-lg font-black text-gray-12">
                  ₱ {expenseSummary.avgTotal.toLocaleString()}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <SummaryCard
              title="Gross Budget"
              amount={expenseSummary.totalExpense}
              count={expenseSummary.totalCount}
              avg={expenseSummary.avgTotal}
              Icon={Calculator}
              colorClass="hover:border-gray-4"
              iconColor="text-gray-12"
            />
            <SummaryCard
              title="Account Tasks"
              amount={expenseSummary.withRefExpense}
              count={expenseSummary.withRefCount}
              avg={expenseSummary.avgWithRef}
              Icon={Tag}
              colorClass="hover:border-gray-4"
              iconColor="text-gray-12"
            />
...
function SummaryCard({
  ...
  return (
    <div
      className={`bg-gray-1 border-2 border-gray-4 rounded-3xl p-6 flex flex-col relative overflow-hidden group ${colorClass} transition-colors`}
    >
      ...
      <div className="flex items-center justify-between mt-auto pt-4 border-t border-gray-3">
        <span className="text-[10px] font-black text-gray-8 uppercase tracking-widest">
          {count} Items
        </span>
        <span
          className={`text-[10px] font-black bg-gray-2 px-2 py-0.5 rounded-full`}
        >
          ₱ {avg}/avg
        </span>
      </div>
