...
import { Trophy, TrendingUp, TrendingDown, Download, AlertCircle, FileText, LayoutDashboard, PieChart, Calendar, Package } from "lucide-react";
...
export default function SalesDashboard() {
  ...
  // === ANALYTICS STATE ===
  const [startDate, setStartDate] = useState(formatDateToYMD(new Date(currentDate.getFullYear(), currentDate.getMonth(), 1))); // default this month
  ...
  const companyPct = totalQuota > 0 ? Math.round((totalWon / totalQuota) * 100) : 0;
  ...
  const anAggs = useMemo(() => {
     ...
     analyticsLogs.forEach(log => {
        ...
        if (isWon) won += val; else lost += val;
        
        // Employee logic
        const empName = log.employees?.name || 'Unknown Rep';
        if (!byEmp[empName]) byEmp[empName] = { won: 0, lost: 0 };
        if (isWon) byEmp[empName].won += val; else byEmp[empName].lost += val;

        // Product logic
        const prod = log.product_item_sold || 'Unknown Product';
        if (!byProd[prod]) byProd[prod] = { won: 0, lost: 0, count: 0 };
        byProd[prod].count += 1;
        if (isWon) byProd[prod].won += val; else byProd[prod].lost += val;
     });

     const empArr = Object.entries(byEmp).map(([name, data]) => ({ name, ...data })).sort((a,b) => b.won - a.won);
     const prodArr = Object.entries(byProd).map(([name, data]) => ({ name, ...data })).sort((a,b) => b.won - a.won);
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 pb-10 px-2 sm:px-0">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-gray-4 pb-4 print:hidden">
        <div>
          <h1 className="text-3xl font-black text-gray-12 flex items-center gap-3">
             <Trophy size={32} className="text-yellow-500" /> Sales Performance
          </h1>
          <p className="text-gray-9 mt-1 font-medium text-sm">Monitor real-time pipeline, quotas, and granular date-ranged analytics.</p>
        </div>
      ...
      <div className="hidden print:block mb-8 border-b-2 border-gray-12 pb-4">
         <h1 className="text-2xl font-black">Sales Department Report ({activeTab === 'OVERVIEW' ? selectedMonth : `${startDate} to ${endDate}`})</h1>
      </div>
      ...
      {activeTab === 'OVERVIEW' && (
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
           ...
           {/* THE LEADERBOARD */}
           <div className="bg-gray-1 border border-gray-4 rounded-2xl shadow-xl overflow-hidden mt-8">
              <div className="bg-gray-2 border-b border-gray-4 p-4 flex justify-between items-center">
                 <h2 className="font-black text-gray-12 text-sm uppercase tracking-wider">Employee Rankings [{selectedMonth}]</h2>
                 {isLdrLoading && <span className="text-[10px] uppercase font-bold text-primary flex items-center gap-1 animate-pulse"><TrendingUp size={12}/> Syncing...</span>}
              </div>
              {leaderboard.length === 0 ? (
                 <div className="p-10 text-center text-gray-9 flex flex-col items-center">
                    <AlertCircle size={32} className="mb-2 opacity-50" />
                    <p className="font-medium">No sales quotas have been distributed for this month yet.</p>
                 </div>
              ) : (
                    ...
                    <table className="w-full text-left border-collapse whitespace-nowrap">
                        ...
                        <tbody className="divide-y divide-gray-4">
                           {leaderboard.map((emp, idx) => {
                              const pct = emp.quota > 0 ? Math.round((emp.revenueWon / emp.quota) * 100) : 0;
                              ...
                              return (
                                 <tr key={emp.name} className={`hover:bg-gray-3 transition-colors ${isTop ? 'bg-yellow-500/5' : ''}`}>
                                    ...
                                    <td className="p-4 text-right font-mono text-green-400 font-black text-sm">
                                       {canSeeNumbers ? `₱${emp.revenueWon.toLocaleString()}` : <span className="text-green-900 font-sans">{pct}% Achieved</span>}
                                    </td>
                                    <td className="p-4 text-right font-mono text-red-400 font-bold text-sm">
                                       {canSeeNumbers ? `₱${emp.revenueLost.toLocaleString()}` : <span className="text-gray-6 italic font-sans">N/A</span>}
                                    </td>
                                    <td className="p-4 text-center">
                                       <div className="flex items-center justify-center gap-2 w-max mx-auto">
                                          <div className="w-24 h-2 bg-gray-4 rounded-full overflow-hidden flex-shrink-0 shadow-inner">
                                             <div className={`h-full ${pct >= 100 ? 'bg-green-500 shadow-[0_0_10px_rgba(34,197,94,0.5)]' : pct >= 50 ? 'bg-yellow-500' : 'bg-red-500'}`} style={{ width: `${Math.min(pct, 100)}%` }}></div>
                                          </div>
                                          <span className="text-xs font-bold text-gray-12 w-8">{pct}%</span>
                                       </div>
                                    </td>
                                 </tr>
                              )
                ...
                <table className="w-full text-left border-collapse">
                   ...
                   <tbody className="divide-y divide-gray-4">
                      {overviewLogs.length === 0 ? (
                         <tr><td colSpan="6" className="p-6 text-center text-gray-9 py-10 font-medium">No sales logged for this month view.</td></tr>
                      ) : (
                         overviewLogs.map((log) => (
                            <tr key={log.id} className="hover:bg-gray-2 transition-colors">
                               <td className="p-3 font-mono text-xs text-gray-11 whitespace-nowrap">{log.date}</td>
                               <td className="p-3 font-bold text-[13px] text-gray-12">{log.employees?.name}</td>
                               <td className="p-3 text-[13px] text-gray-12 font-medium truncate max-w-[200px]" title={log.account}>{log.account}</td>
                               <td className="p-3 text-[13px] text-gray-11 truncate max-w-[200px]">{log.product_item_sold}</td>
                               <td className="p-3 font-mono text-sm text-right font-black text-gray-12">{Number(log.revenue_amount).toLocaleString()}</td>
      ...
      {activeTab === 'ANALYTICS' && (
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-6">
           ...
           <div className="bg-gray-1 border border-gray-4 rounded-2xl p-6 shadow-sm print:hidden">
              <h2 className="text-xs font-black uppercase tracking-widest text-gray-10 mb-4 flex items-center gap-2"><Calendar size={14} /> Custom Date Boundary</h2>
              
              <div className="flex flex-col lg:flex-row gap-6 lg:items-end justify-between">
                 <div className="flex flex-wrap gap-4 items-end">
                    <div>
                       <label className="text-[10px] font-bold text-gray-8 uppercase tracking-wider block mb-1">Start Date</label>
                       <input type="date" value={startDate} onChange={e => { setStartDate(e.target.value); setActivePreset(null); }} className="bg-gray-2 border border-gray-4 rounded-lg px-3 py-2 text-sm text-gray-12 font-bold outline-none focus:border-blue-500 w-[140px]" />
                    </div>
                    <div>
                       <label className="text-[10px] font-bold text-gray-8 uppercase tracking-wider block mb-1">End Date</label>
                       <input type="date" value={endDate} onChange={e => { setEndDate(e.target.value); setActivePreset(null); }} className="bg-gray-2 border border-gray-4 rounded-lg px-3 py-2 text-sm text-gray-12 font-bold outline-none focus:border-blue-500 w-[140px]" />
                    </div>
                 </div>
                 
                 <div className="flex gap-1 bg-gray-2 p-1 rounded-xl border border-gray-4 max-w-full overflow-x-auto">
                    <button onClick={() => handleAnalyticPreset('TODAY')} className={`px-4 py-2 text-xs font-bold uppercase tracking-wider whitespace-nowrap rounded-lg transition-colors ${activePreset === 'TODAY' ? 'bg-gray-4 text-gray-12 shadow-sm' : 'text-gray-9 hover:text-gray-12 hover:bg-gray-3'}`}>Today</button>
                    <button onClick={() => handleAnalyticPreset('THIS_WEEK')} className={`px-4 py-2 text-xs font-bold uppercase tracking-wider whitespace-nowrap rounded-lg transition-colors ${activePreset === 'THIS_WEEK' ? 'bg-gray-4 text-gray-12 shadow-sm' : 'text-gray-9 hover:text-gray-12 hover:bg-gray-3'}`}>This Week</button>
                    <button onClick={() => handleAnalyticPreset('THIS_MONTH')} className={`px-4 py-2 text-xs font-bold uppercase tracking-wider whitespace-nowrap rounded-lg transition-colors ${activePreset === 'THIS_MONTH' ? 'bg-gray-4 text-gray-12 shadow-sm' : 'text-gray-9 hover:text-gray-12 hover:bg-gray-3'}`}>This Month</button>
                          ...
                          <div key={emp.name} className="flex justify-between items-center bg-gray-2 p-3 rounded-xl border border-gray-3 hover:border-gray-4 transition-colors">
                             <div className="flex items-center gap-3">
                                ...
                                <div>
                                   ...
                                   <p className="text-[10px] uppercase font-bold text-gray-8 tracking-wider">Lost: ₱{emp.lost.toLocaleString()}</p>
                                </div>
                             </div>
                 ...
                 <div className="space-y-4">
                    {anAggs.prodArr.length === 0 ? (
                       ...
                       anAggs.prodArr.map((prod, i) => (
                          <div key={prod.name} className="bg-gray-2 p-4 rounded-xl border border-gray-3 transition-transform hover:-translate-y-0.5 shadow-sm">
                             <div className="flex justify-between items-start mb-2">
                                <p className="text-sm font-bold text-gray-12 truncate block pr-2" title={prod.name}>{prod.name}</p>
