...
const Dropdown = forwardRef(({
  ...
  return (
    <div className={`relative ${className}`} ref={(node) => {
      dropdownRef.current = node;
      if (typeof ref === 'function') ref(node);
      else if (ref) ref.current = node;
    }}>
      <div onClick={handleToggle} className={`w-full ${disabled ? 'cursor-not-allowed opacity-60' : 'cursor-pointer'}`}>
        ...
        usePortal ? createPortal(
          <div 
            ...
            style={{ 
              ...
              // Respect placement
              ...(placement.endsWith("end") ? { right: coords?.right, left: "auto" } : { left: coords?.left, right: "auto" }),
              minWidth: coords?.width 
            }}
          >
            {typeof children === "function" ? children({ close: () => isControlled ? onClose?.() : setInternalIsOpen(false) }) : children}
          </div>,
          document.body
        ) : (
          <div ref={popoverRef} className={`${popoverClassName} ${placementClass}`}>
            {typeof children === "function" ? children({ close: () => isControlled ? onClose?.() : setInternalIsOpen(false) }) : children}
          </div>
        )
      )}
