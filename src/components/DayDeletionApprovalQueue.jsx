...
import { Trash2, Calendar, AlertCircle, CheckCircle2, XCircle } from "lucide-react";

export default function DayDeletionApprovalQueue() {
  const { user } = useAuth();
  ...
  const resolveMutation = useMutation({
    mutationFn: ({ employeeId, date, isApproved }) => salesService.resolveDayDeletion(employeeId, date, isApproved, user.id),
    onSuccess: () => {
    ...
    <div className="mb-8 space-y-4">
      <div className="flex items-center justify-between mb-4 border-b border-gray-4 pb-2">
         <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-black text-gray-12 uppercase tracking-tight">Day Deletion Requests</h2>
         </div>
      ...
      {requests.map(req => (
         <div key={`${req.employee_id}_${req.date}`} className="bg-gray-1 border border-red-500/20 rounded-2xl overflow-hidden shadow-sm hover:border-red-500/40 transition-all group">
            <div className="bg-red-500/5 p-4 sm:p-6 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
               <div className="flex items-center gap-4 shrink-0">
                  ...
                  <div>
                     <h3 className="font-bold text-gray-12 text-lg leading-tight uppercase tracking-tight">{req.employeeName}</h3>
               ...
               <div className="flex items-center gap-3 shrink-0 self-end lg:self-center">
                  <button 
                    ...
                    onClick={() => resolveMutation.mutate({ employeeId: req.employee_id, date: req.date, isApproved: false })}
                  ...
                  <button 
                    disabled={resolveMutation.isPending}
                    onClick={() => {
                        if (window.confirm(`Are you sure you want to PERMANENTLY delete all ${req.activities.length} activities for ${req.employeeName} on ${req.date}?`)) {
                            resolveMutation.mutate({ employeeId: req.employee_id, date: req.date, isApproved: true });
                        }
                    }}
